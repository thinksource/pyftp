...
class FtpConnection(threading.Thread):  
    ...
    def process(self, cmd, arg):  
        ...
        if cmd == "BYE" or cmd == "QUIT":  
            if os.path.exists(root_dir + "/pyftp.goodbye"):  
                self.message(221, open(root_dir + "/pyftp.goodbye").read())  
            else:  
            ...
            if not os.path.isdir(self.home_dir):  
                ...
                return  
            self.pass_path = self.home_dir + "/.pyftp/password"  
        ...
        try:  
            if cmd == "CWD":  
                ...
                if os.path.exists(newpath):  
                    self.message(213, time.strftime("%Y%m%d%I%M%S", time.localtime(os.path.getmtime(newpath))))  
                else:  
                    self.message(550, "failed")  
            elif cmd == "SIZE":  
    ...
    def run(self):  
        ...
        try:  
            if len(conn_list) > max_connections:  
                ...
                return  
            # Welcome Message  
            if os.path.exists(root_dir + "/pyftp.welcome"):  
                self.message(220, open(root_dir + "/pyftp.welcome").read())  
            else:  
                self.message(220, "pyftp(Python)")  
            # Command Loop  
            line = ""  
            while self.running:  
